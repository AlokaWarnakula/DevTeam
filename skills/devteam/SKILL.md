...
- **A blocked task cannot be lifted by any agent, and that is the design — not a missing feature.** When a room you are in is blocked, `devteam_why_blocked`, `devteam_wait` and `devteam_state` all carry a `blockedRooms` / `blockedRecovery` block naming the task, the reason it was stopped, and the exact action the human takes. Relay that to the human in one sentence — *which* task to resume and *why* — and then wait. **Do not delete and recreate the task, do not open a duplicate task for the same work, and do not approve or accept anything to route around the block.** Recreating a task throws away its version history, approvals, findings and reliability record to work around a control the human simply has not found yet. If the human says there is no such control, tell them where it is: the **"Task blocked" banner directly under the task title**, or **"Resume blocked task"** at the foot of the team panel. They can also name an agent when resuming, which addresses the fresh planning assignment to that agent by name.
...
- **`devteam_block` is one-way for you: only the human can resume a blocked task.** It closes every open assignment on the task, cancels ready checkpoints, and stands the room down. Nothing you can call reopens it — the human resumes it from the dashboard, which reopens the task at the next version, clears the approvals recorded against the current one, and queues a single fresh planning assignment. So write a `reason` the human can act on without you: what decision or authorization you need, and what should happen once they give it. Blocking with a vague reason strands the task behind a human who does not know what you want.
