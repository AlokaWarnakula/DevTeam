...
- Use `dependsOn` with existing same-task assignment IDs when work has a real prerequisite. DevTeam keeps the dependent assignment queued until every prerequisite is done, so planners do not need to manually create each stage later.
